...
#include "simulation/predator/predator.hpp" // MS02: Raeuber-Feature


struct Simulation
{
    civ::Vector<Colony> colonies;
    std::vector<Predator> predators; // MS02: Liste aller aktiven Raeuber auf der Karte
    World world;
    // Render
    Renderer renderer;
    EventSate ev_state;
        ...
        , distance_field_builder(world.map)
    {
        ...
        // predators.emplace_back(400.0f, 300.0f); // MS02: Testzeile entfernt - Platzierung erfolgt ueber UI
    }
    ...
    void update(float dt)
    {
        if (!ev_state.pause) {
            ...
            // MS02: Alle Raeuber aktualisieren - laeuft nach den Ameisen,
            // damit Raeuber auf die Positionen des aktuellen Ticks reagieren
            for (Predator& p : predators) {
                p.update(colonies, world, dt);
            }

            // MS02: Raeuber beschaedigt die Koenigin, wenn er nah genug ist.
            // Verbindet das Raeuber-Feature (IZ) mit dem Koeniginnen-Feature (NA):
            // Stirbt die Koenigin, stoppt das Koloniewachstum (siehe Colony::createNewAnts).
            for (Colony& colony : colonies) {
                if (!colony.queen.isAlive()) { continue; }
                for (Predator& p : predators) {
                    if (getLength(p.position - colony.queen.position) < Queen::damage_radius) {
                        colony.queen.takeDamage(60.0f * dt); // Schaden pro Sekunde solange Raeuber in Reichweite
                    }
                }
            }
            // Update stats
    ...
    void render(sf::RenderTarget& target)
    {
        ...
        // MS02: RenderStates des ViewportHandlers weitergeben,
        // damit Zoom und Pan korrekt auf Raeuber und Koeniginnen angewendet werden
        const sf::RenderStates states = renderer.vp_handler.getRenderState();
        // MS02: Raeuber zeichnen
        for (const Predator& p : predators) {
            p.render(target, states);
        }
        // MS02: Koenigin jeder Kolonie zeichnen
        for (Colony& colony : colonies) {
            colony.queen.render(target, states);
        }
    }
