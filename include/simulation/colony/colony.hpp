...
#include "simulation/Queen/Queen.hpp" // MS02: Koeniginnen-Feature - steuert ob Koloniewachstum moeglich ist

struct Colony
{
	ColonyBase       base;
	Queen			 queen;            // MS02: Koenigin der Kolonie, sitzt im Nest - stirbt sie, stoppt das Wachstum
	...
	void initialize(uint8_t colony_id)
	{
		id = colony_id;
		base.food = 0.0f;
		queen.position = base.position; // MS02: Koenigin wird beim Start im Nest platziert
		queen.alive = true;          // MS02: Koenigin startet lebendig
	...
	{
		queen.position = new_position;  // MS02: Koenigin bleibt beim Nest wenn Kolonie verschoben wird
	...
	{
		// MS02: Ohne lebende Koenigin koennen keine neuen Ameisen erschaffen werden.
		// Wird die Koenigin vom Raeuber getoetet (siehe Simulation::update), stirbt die
		// Kolonie langsam aus, da keine neuen Ameisen nachruecken.
