...
struct ColonyRenderer
{
	...
	void renderAnts(sf::RenderTarget& target, const sf::RenderStates& states)
	{
        ...
        // MS02: Soldiers heller als Workers färben (gleicher Grundton, Richtung Weiß
        // aufgehellt), damit man die beiden Typen optisch unterscheiden kann
        const float soldier_brighten = 0.5f;
        const sf::Color soldier_color = ColorUtils::createColor(
            colony.ants_color.r + soldier_brighten * (255 - colony.ants_color.r),
            colony.ants_color.g + soldier_brighten * (255 - colony.ants_color.g),
            colony.ants_color.b + soldier_brighten * (255 - colony.ants_color.b));

		uint32_t index = 0;
		for (const Ant& a : colony.ants) {
            const uint32_t va_index = 4 * index;
            const sf::Color& ant_color = (a.type == Ant::Type::Soldier) ? soldier_color : colony.ants_color;
            ants_va[va_index + 0].color = ant_color;
            ants_va[va_index + 1].color = ant_color;
            ants_va[va_index + 2].color = ant_color;
            ants_va[va_index + 3].color = ant_color;
			a.render_in(ants_va, va_index);
			a.render_food(ants_food_va, va_index);
			++index;
		}
