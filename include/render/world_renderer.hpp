		...
		for (int32_t y(0); y < grid.height; y++) {
			for (int32_t x(0); x < grid.width; x++) {
				...
				if (!cell.food && !cell.wall) {
					...
					// MS02: der äußerste Zellring ist die Weltgrenze, kein vom Nutzer
					// platzierter Wand-Pinselstrich - die wird nicht mehr sichtbar gerendert,
					// weil ihre mit dem Zoom mitskalierende Dicke nicht zur fixen UI-Rahmenlinie
					// passt (siehe editor_scene.hpp). Innenliegende Wände bleiben sichtbar.
					const bool is_world_border = (x == 0 || x == grid.width - 1 || y == 0 || y == grid.height - 1);
					if (is_world_border) {
