...
public:
    ...
    RoundedRectangle(sf::Vector2f size, sf::Vector2f position, float radius)
        : m_width(size.x)
        , m_height(size.y)
        // MS02: nie negativ - sonst erzeugt sf::CircleShape an den Ecken kaputte
        // Mini-Artefakte (z.B. bei radius - select_padding < 0)
        , m_radius(std::max(0.0f, std::min(0.5f * std::min(m_width, m_height), radius)))
        , m_x(position.x)
        , m_y(position.y)
        , m_color(sf::Color::White)
    {}
