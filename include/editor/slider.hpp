...
struct Slider : public GUI::Item
{
    ...
    {
        const float slider_height = 16.0f; // MS02: etwas flacher
    ...
    {
        const sf::Color color(140, 140, 140); // MS02: heller, da Hintergrund jetzt transparent/schwarz ist
        // Bar
...
struct SliderLabel : public GUI::Container
{
    ...
    SPtr<GUI::TextLabel> name_label;

    // MS02: name_ wird mit reingerendert (Hintergrund-Box dient jetzt als komplettes
    // "Size: <Wert> <Slider>"-Element statt einer separaten Sektionsüberschrift davor)
    SliderLabel(const std::string& name_, float max_value_, float min_value_ = 0.0f, sf::Vector2f size_ = {}, sf::Vector2f position_ = {})
        : GUI::Container(GUI::Container::Orientation::Horizontal, size_, position_)
    {
        size_type.y = GUI::Size::FitContent;

        name_label = create<GUI::TextLabel>(name_, 12);
        name_label->setAlignment(GUI::Alignment::Left);
        name_label->setColor(sf::Color(200, 200, 200)); // MS02: hellgraue Schrift
        addItem(name_label);

        slider = create<Slider>(max_value_, min_value_);
        label = create<GUI::TextLabel>("", 12);
        label->setWidth(20.0f);
        label->auto_size_update = false;
        label->setColor(sf::Color(200, 200, 200)); // MS02: hellgraue Schrift
    ...
    void render(sf::RenderTarget& target) override
    {
        // MS02: transparente Füllung + grauer Rahmen statt grauer Füllung (gleicher
        // Look wie alle Buttons)
