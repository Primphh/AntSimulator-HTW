...
struct WorldView : GUI::Item
{
    ...
    void render(sf::RenderTarget& target) override
    {
        ...
        // MS02: View so aufbauen, dass lokale Koordinaten (0,0)..size weiterhin an der
        // tatsächlichen GUI-Position landen (wie vorher), ZUSÄTZLICH aber per setViewport()
        // hart auf genau dieses Rechteck geclippt wird - zoomt man über den "contain"-Fit
        // hinaus, wird der Überstand jetzt am grünen Rahmen abgeschnitten statt drüber zu malen.
        const sf::View previous_view = target.getView();
        const sf::Vector2u target_size = target.getSize();
        sf::View clipped_view;
        clipped_view.reset(sf::FloatRect(0.0f, 0.0f, size.x * Conf::GUI_SCALE, size.y * Conf::GUI_SCALE));
        clipped_view.setViewport(sf::FloatRect(
            position.x * Conf::GUI_SCALE / to<float>(target_size.x),
            position.y * Conf::GUI_SCALE / to<float>(target_size.y),
            size.x * Conf::GUI_SCALE / to<float>(target_size.x),
            size.y * Conf::GUI_SCALE / to<float>(target_size.y)
        ));
        target.setView(clipped_view);
    ...
    void update() override
    {
        ...
        if (current_time_state == TimeController::State::Play) {
            simulation.update(dt * control_state.simulation_speed);
        }
    }
};
